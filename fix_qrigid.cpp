...
FixQRigid::FixQRigid(LAMMPS *lmp, int narg, char **arg) :
	...
	/*--------------------------------------------------------------------------------*/
{
  ...
  if (narg < 4) error->all(FLERR,"Illegal fix qrigid command");
  int iarg;
  ...
  // use nall as incremented ptr to set body[] values for each atom
    ...
    if (custom_flag) {
      if (narg < 5) error->all(FLERR,"Illegal fix qrigid command");
      ...
      if (strstr(arg[4],"i_") == arg[4]) {
        ...
        if (custom_index == -1)
          error->all(FLERR,"Fix qrigid custom requires previously defined property/atom");
        else if (is_double)
          error->all(FLERR,"Fix qrigid custom requires integer-valued property/atom");
        int minval = INT_MAX;
        ...
        if (ivariable < 0)
          error->all(FLERR,"Variable name for fix qrigid custom does not exist");
        if (input->variable->atomstyle(ivariable) == 0)
          error->all(FLERR,"Fix qrigid custom variable is no atom-style variable");
        ...
        delete[] value;
      } else error->all(FLERR,"Unsupported fix qrigid custom property");
    } else {
      if (atom->molecule_flag == 0)
        error->all(FLERR,"Fix qrigid molecule requires atom attribute molecule");
    ...
    if (itmp+1 > MAXSMALLINT)
      error->all(FLERR,"Too many molecules for fix qrigid");
    maxmol = (int) itmp;

    int *ncount;
    memory->create(ncount,maxmol+1,"qrigid:ncount");
    for (i = 0; i <= maxmol; i++) ncount[i] = 0;

    for (i = 0; i < nlocal; i++)
      if (mask[i] & groupbit) ncount[molecule[i]]++;

    memory->create(mol2body,maxmol+1,"qrigid:mol2body");
    ...
    memory->create(body2mol,nbody,"qrigid:body2mol");
  ...
  // error if atom belongs to more than 1 rigid body
    ...
    if (nbody <= 0) error->all(FLERR,"Illegal fix qrigid command");
    if (narg < 5+nbody) error->all(FLERR,"Illegal fix qrigid command");
    iarg = 5+nbody;
    ...
    for (ibody = 0; ibody < nbody; ibody++) {
      ...
      if (igroups[ibody] == -1)
        error->all(FLERR,"Could not find fix qrigid group ID");
    }
  ...
  } else error->all(FLERR,"Illegal fix qrigid command");
  ...
  memory->create(nrigid,nbody,"qrigid:nrigid");
  memory->create(masstotal,nbody,"qrigid:masstotal");
  memory->create(xcm,nbody,3,"qrigid:xcm");
  memory->create(vcm,nbody,3,"qrigid:vcm");
  memory->create(fcm,nbody,3,"qrigid:fcm");
  memory->create(inertia,nbody,3,"qrigid:inertia");
  memory->create(ex_space,nbody,3,"qrigid:ex_space");
  memory->create(ey_space,nbody,3,"qrigid:ey_space");
  memory->create(ez_space,nbody,3,"qrigid:ez_space");
  memory->create(angmom,nbody,3,"qrigid:angmom");
  memory->create(omega,nbody,3,"qrigid:omega");
  memory->create(torque,nbody,3,"qrigid:torque");
  memory->create(quat,nbody,4,"qrigid:quat");
  memory->create(imagebody,nbody,"qrigid:imagebody");
  memory->create(fflag,nbody,3,"qrigid:fflag");
  memory->create(tflag,nbody,3,"qrigid:tflag");
	/*--------------------------------------------------------------------------------*/
	// QOCO
	memory->create(qocoextra,nbody,6,"qrigid:qocoextra");
 	/*--------------------------------------------------------------------------------*/
  memory->create(langextra,nbody,6,"qrigid:langextra");
  memory->create(sum,nbody,6,"qrigid:sum");
  memory->create(all,nbody,6,"qrigid:all");
  memory->create(remapflag,nbody,4,"qrigid:remapflag");
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"force") == 0) {
      if (iarg+5 > narg) error->all(FLERR,"Illegal fix qrigid command");
      ...
      if (domain->dimension == 2 && zflag == 1.0)
        error->all(FLERR,"Fix qrigid z force cannot be on for 2d simulation");
      ...
      if (count == 0) error->all(FLERR,"Illegal fix qrigid command");
      ...
      if (domain->dimension == 2 && (xflag == 1.0 || yflag == 1.0))
        error->all(FLERR,"Fix qrigid xy torque cannot be on for 2d simulation");
      ...
      if (iarg+5 > narg) error->all(FLERR,"Illegal fix qrigid command");
      if (strcmp(style,"qrigid") != 0 && strcmp(style,"qrigid/nve") != 0 &&
          strcmp(style,"qrigid/omp") != 0 && strcmp(style,"qrigid/nve/omp") != 0)
       error->all(FLERR,"Illegal fix qrigid command");
      langflag = 1;
      ...
      if (t_period <= 0.0)
        error->all(FLERR,"Fix qrigid langevin period must be > 0.0");
      if (seed <= 0) error->all(FLERR,"Illegal fix qrigid command");
      iarg += 5;
		/*--------------------------------------------------------------------------------*/
		// QOCO
		}	else if (strcmp(arg[iarg],"qoco") == 0) {
		/* ---------------------------------------------------------------------- */
      qocoflag = 1;
      if (iarg+1 > narg) error->all(FLERR,"Illegal fix qrigid command");
			/* ---------------------------------------------------------------------- */
			// Get compute qoco ID: Define the force/torque per-rigid body;
			/* ---------------------------------------------------------------------- */
			int n = strlen(arg[iarg+1]) + 1;
			idalpha = new char[n];
			strcpy(idalpha,arg[iarg+1]);
      iarg += 2;
 	/*--------------------------------------------------------------------------------*/
...
int FixQRigid::setmask()
{
  ...
  mask |= FINAL_INTEGRATE;
	/*--------------------------------------------------------------------------------*/
	// QOCO
  if (langflag || qocoflag) mask |= POST_FORCE;
 	/*--------------------------------------------------------------------------------*/
  mask |= PRE_NEIGHBOR;
...
void FixQRigid::init()
{
  ...
  if (ndof > 0.0) tfactor = force->mvv2e / (ndof * force->boltz);
	...
	/* ---------------------------------------------------------------------- */
	// Check QOCO
	int icompute = modify->find_compute(idalpha);
	if (icompute < 0)
		error->all(FLERR,"Compute qoco does not exist for fix qrigid");
	calf = (ComputeQOCO *) modify->compute[icompute];
	if (strcmp(calf->style,"qoco") != 0)
		error->all(FLERR,"Fix qrigid does not use compute qoco");
 	/*--------------------------------------------------------------------------------*/
}
...
void FixQRigid::setup_pre_neighbor()
{
  pre_neighbor();
}
...
void FixQRigid::setup(int vflag)
{
  int i,n,ibody;
  ...
  for (ibody = 0; ibody < nbody; ibody++)		{
    for (i = 0; i < 6; i++)	{
			langextra[ibody][i] = 0.0;
	/*--------------------------------------------------------------------------------*/
	// QOCO
    	qocoextra[ibody][i] = 0.0;
 	/*--------------------------------------------------------------------------------*/
		}
	}
...
void FixQRigid::initial_integrate(int vflag)
{
  double dtfm;
...
void FixQRigid::apply_langevin_thermostat()
{
  if (me == 0) {
...
/* ----------------------------------------------------------------------
   apply QOCO method to all rigid bodies
   computed by proc 0, broadcast to other procs
   This stores extra force in extra arrays,
     which are added in when final_integrate() calculates a new fcm/torque
------------------------------------------------------------------------- */

void FixQRigid::apply_qoco()
{
 // calculate current alpha for and forces/torques on each body
	calf->fcompute_qoco();
}
...
void FixQRigid::enforce2d()
{
  for (int ibody = 0; ibody < nbody; ibody++) {
	...
	/*--------------------------------------------------------------------------------*/
	// QOCO
    if (qocoflag && qocoextra) {
      qocoextra[ibody][2] = 0.0;
      qocoextra[ibody][3] = 0.0;
      qocoextra[ibody][4] = 0.0;
    }
 	/*--------------------------------------------------------------------------------*/
  }
}
...
void FixQRigid::compute_forces_and_torques()
{
  int i,ibody;
  ...
  // include Langevin thermostat forces
	...
	// QOCO
  for (ibody = 0; ibody < nbody; ibody++) {
    fcm[ibody][0]			= all[ibody][0] + langextra[ibody][0] + qocoextra[ibody][0];
    fcm[ibody][1]			= all[ibody][1] + langextra[ibody][1] + qocoextra[ibody][1];
    fcm[ibody][2]			= all[ibody][2] + langextra[ibody][2] + qocoextra[ibody][2];
    torque[ibody][0]	= all[ibody][3] + langextra[ibody][3] + qocoextra[ibody][3];
    torque[ibody][1]	= all[ibody][4] + langextra[ibody][4] + qocoextra[ibody][4];
    torque[ibody][2]	= all[ibody][5] + langextra[ibody][5] + qocoextra[ibody][5];
 	/*--------------------------------------------------------------------------------*/
  }
}
...
void FixQRigid::post_force(int /*vflag*/)
{
  if (langflag) apply_langevin_thermostat();
	/*--------------------------------------------------------------------------------*/
	// QOCO
  if (qocoflag)	calf->fcompute_qoco();	//apply_qoco();
 	/*--------------------------------------------------------------------------------*/
...
void FixQRigid::final_integrate()
{
  int ibody;
  double dtfm;
...
void FixQRigid::initial_integrate_respa(int vflag, int ilevel, int /*iloop*/)
{
...
void FixQRigid::final_integrate_respa(int ilevel, int /*iloop*/)
{
  dtf = 0.5 * step_respa[ilevel] * force->ftm2v;
  final_integrate();
}
...
void FixQRigid::deform(int flag)
{
  if (flag == 0)
    for (int ibody = 0; ibody < nbody; ibody++)
...
void FixQRigid::setup_bodies_static()
{
  int i,ibody;
  ...
  int *inbody;
  if (infile) {
    memory->create(inbody,nbody,"qrigid:inbody");
  ...
  for (ibody = 0; ibody < nbody; ibody++) {
    ...
    if (inertia[ibody][0] == 0.0) {
      if (fabs(all[ibody][0]) > TOLERANCE)
        error->all(FLERR,"Fix qrigid: Bad principal moments");
    } else {
      if (fabs((all[ibody][0]-inertia[ibody][0])/inertia[ibody][0]) >
          TOLERANCE) error->all(FLERR,"Fix qrigid: Bad principal moments");
    }
...
void FixQRigid::setup_bodies_dynamic()
{
  int i,ibody;
...
void FixQRigid::readfile(int which, double *vec,
...
void FixQRigid::write_restart_file(char *file)
{
  if (me) return;
...
void FixQRigid::grow_arrays(int nmax)
{
  memory->grow(body,nmax,"qrigid:body");
  memory->grow(xcmimage,nmax,"qrigid:xcmimage");
  memory->grow(displace,nmax,3,"qrigid:displace");
  if (extended) {
    memory->grow(eflags,nmax,"qrigid:eflags");
    if (orientflag) memory->grow(orient,nmax,orientflag,"qrigid:orient");
    if (dorientflag) memory->grow(dorient,nmax,3,"qrigid:dorient");
  }
...
void FixQRigid::copy_arrays(int i, int j, int /*delflag*/)
{
  body[j] = body[i];
...
void FixQRigid::set_arrays(int i)
{
  body[i] = -1;
  xcmimage[i] = 0;
  displace[i][0] = 0.0;
...
int FixQRigid::pack_exchange(int i, double *buf)
{
  buf[0] = ubuf(body[i]).d;
...
int FixQRigid::unpack_exchange(int nlocal, double *buf)
{
...
int FixQRigid::modify_param(int narg, char **arg)
{
  if (strcmp(arg[0],"bodyforces") == 0) {
    ...
    for (int i = 0; i < modify->nfix; i++)
      if (strcmp(modify->fix[i]->id,id) == 0) {
        ...
        else if (!langflag || !qocoflag) modify->fmask[i] &= ~POST_FORCE;
        break;
      }
    return 2;
  }
...
double FixQRigid::compute_scalar()
{
...
void *FixQRigid::extract(const char *str, int &dim)
{
...
double FixQRigid::extract_erotational()
{
  double wbody[3],rot[3][3];
...
double FixQRigid::compute_array(int i, int j)
{
