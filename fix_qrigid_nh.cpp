...
enum{NONE,XYZ,XY,YZ,XZ};     // same as in FixQRigid
enum{ISO,ANISO,TRICLINIC};   // same as in FixQRigid

#define EPSILON 1.0e-7

/* ---------------------------------------------------------------------- */

FixQRigidNH::FixQRigidNH(LAMMPS *lmp, int narg, char **arg) :
  FixQRigid(lmp, narg, arg), conjqm(NULL), w(NULL),
...
{
  // error checks: could be moved up to FixQRigid
  ...
  memory->create(conjqm,nbody,4,"qrigid_nh:conjqm");
...
int FixQRigidNH::setmask()
{
  int mask = 0;
  mask = FixQRigid::setmask();
...
void FixQRigidNH::setup(int vflag)
{
  FixQRigid::setup(vflag);
...
void FixQRigidNH::initial_integrate(int vflag)
{
...
void FixQRigidNH::final_integrate()
{
  int ibody;
...
void FixQRigidNH::nhc_temp_integrate()
{
  int i,j,k;
...
void FixQRigidNH::nhc_press_integrate()
{
  int i,j,k;
...
double FixQRigidNH::compute_scalar()
{
  const double kt = boltz * t_target;
  double energy;
  int i;

  energy = FixQRigid::compute_scalar();
...
void FixQRigidNH::compute_temp_target()
{
...
void FixQRigidNH::compute_press_target()
{
  double delta = update->ntimestep - update->beginstep;
  if (delta != 0.0) delta /= update->endstep - update->beginstep;
...
void FixQRigidNH::nh_epsilon_dot()
{
  int i;
...
void FixQRigidNH::write_restart(FILE *fp)
{
  ...
  double *list;
  memory->create(list,nsize,"qrigid_nh:list");
...
void FixQRigidNH::restart(char *buf)
{
  int n = 0;
...
int FixQRigidNH::modify_param(int narg, char **arg)
{
  ...
  return FixQRigid::modify_param(narg,arg);
}

/* ---------------------------------------------------------------------- */

void FixQRigidNH::allocate_chain()
{
  if (tstat_flag) {
...
void FixQRigidNH::reset_target(double t_new)
{
...
void FixQRigidNH::allocate_order()
{
...
void FixQRigidNH::deallocate_chain()
{
  if (tstat_flag) {
    delete [] q_t;
    delete [] q_r;
...
void FixQRigidNH::deallocate_order()
{
  delete [] w;
  delete [] wdti1;
  delete [] wdti2;
