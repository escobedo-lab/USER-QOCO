...
FixStyle(qrigid,FixQRigid)

#else

#ifndef LMP_FIX_QRIGID_H
#define LMP_FIX_QRIGID_H

#include "fix.h"

namespace LAMMPS_NS {

class FixQRigid : public Fix {
	friend class ComputeQOCO;
	friend class FixUmbrellaOrient;

 public:
  FixQRigid(class LAMMPS *, int, char **);
  virtual ~FixQRigid();
  virtual int setmask();
  virtual void init();
 ...
 protected:
  ...
  void readfile(int, double *, double **, double **, double **,
	...
	/*--------------------------------------------------------------------------------*/
	// QOCO
  double **qocoextra;       // Langevin thermostat forces and torques
  int qocoflag;             // 0/1 = no/yes QOCO

  char *idalpha;
	class ComputeQOCO *calf;	// Auxilliary compute qoco class

  void apply_qoco();				// Calculate the external force
	/*--------------------------------------------------------------------------------*/
};
